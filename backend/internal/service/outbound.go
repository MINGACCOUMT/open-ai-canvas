...
// CANVAS_ALLOWED_PRIVATE_UPSTREAM_HOSTS 除精确主机名外还支持通配后缀：
//   - *.example.com 匹配 example.com 及其所有子域
//   - .example.com 只匹配子域（不含 apex）
//
// 用于本地开发（Clash fake-IP 会把可信公网域名解析进内网段）精确放行可信
// 云厂商 / CDN 域名，不得用 CANVAS_ALLOW_PRIVATE_UPSTREAMS=true 整体关闭防护替代。
