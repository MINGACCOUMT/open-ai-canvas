...
const capabilityChoices: Array<{
    ...
    { value: "image", label: "图片", description: "生成与编辑", icon: <Image className="size-4" />, brands: ["openai", "gemini", "grok"] },
...
function ProtocolBrandMark({ protocol, compact = false }: { protocol: ModelProtocolDefinition; compact?: boolean }) {
    ...
    const brand = protocol.value === "gemini-veo" ? "gemini" : protocol.value === "xai-image" || protocol.value === "grok-image" || protocol.value === "xai-video" ? "grok" : "openai";
